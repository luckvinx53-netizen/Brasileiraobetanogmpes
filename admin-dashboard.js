...
async function iniciarDashboardAdmin() {
  ...
  // Aplica o tema de cor da competição ativa (mesma lógica pública de
  // competicoes.js), sem depender do slot de topbar pública — o admin
  // tem sua própria topbar/sidebar e não usa #seletorCompeticaoSlot.
  aplicarTemaCompeticaoNoAdmin();
...
async function aplicarTemaCompeticaoNoAdmin() {
  try {
    if (typeof getCompeticaoAtual !== "function" || typeof aplicarTemaCompeticao !== "function") return;
    const atual = await getCompeticaoAtual();
    if (atual) aplicarTemaCompeticao(atual);
  } catch (e) {
    console.warn("Dashboard: falha ao aplicar tema da competição", e);
  }
}
