...
// Soma consultas de transferência e propostas de patrocínio, já que as
// duas aparecem juntas na mesma caixa de Email.
async function contarNotificacoesNaoLidas(timeId) {
  const { count, error } = await supabaseClient
    ...
    .eq("lida", false)
    .is("arquivada_em", null); // negociações arquivadas não devem gerar bolinha de notificação

  if (error) { console.error(error); return 0; }

  const { count: countPatrocinios, error: erroPatrocinios } = await supabaseClient
    .from("patrocinios")
    .select("id", { count: "exact", head: true })
    .eq("time_id", timeId)
    .eq("lida", false)
  ...
  if (erroPatrocinios) { console.error(erroPatrocinios); return count || 0; }

  return (count || 0) + (countPatrocinios || 0);
}
