...
async function salvarJogo() {
  ...
  await salvarArbitragemDoJogo(jogoId);
...
// Salva (insere ou atualiza) o quarteto de arbitragem definido pro jogo.
// Usa upsert por jogo_id (a tabela tem UNIQUE(jogo_id)) — se todos os campos
// estiverem vazios, não grava nada.
async function salvarArbitragemDoJogo(jogoId) {
  const arbitragem = {
    jogo_id: jogoId,
    arbitro: document.getElementById("arbitroJogo").value.trim(),
    assistente_1: document.getElementById("assistente1Jogo").value.trim(),
    assistente_2: document.getElementById("assistente2Jogo").value.trim(),
    quarto_arbitro: document.getElementById("quartoArbitroJogo").value.trim(),
  };

  const algumPreenchido = arbitragem.arbitro || arbitragem.assistente_1 || arbitragem.assistente_2 || arbitragem.quarto_arbitro;
  if (!algumPreenchido) return;

  const { error } = await supabaseClient
    .from("arbitragem_jogo")
    .upsert(arbitragem, { onConflict: "jogo_id" });

  if (error) notificar("Jogo salvo, mas houve erro ao salvar a arbitragem: " + error.message, "erro");
}
...
function editarJogo(j) {
  ...
  document.getElementById("arbitroJogo").value = "";
  document.getElementById("assistente1Jogo").value = "";
  document.getElementById("assistente2Jogo").value = "";
  document.getElementById("quartoArbitroJogo").value = "";
...
// Busca o quarteto de arbitragem já cadastrado para este jogo (se houver)
// e preenche os campos correspondentes na sub-aba "Dados".
async function carregarArbitragemDoJogo(jogoId) {
  ...
  // Só preenche se o usuário ainda estiver no mesmo jogo (evita corrida caso
  ...
  document.getElementById("arbitroJogo").value = data.arbitro || "";
  document.getElementById("assistente1Jogo").value = data.assistente_1 || "";
  document.getElementById("assistente2Jogo").value = data.assistente_2 || "";
  document.getElementById("quartoArbitroJogo").value = data.quarto_arbitro || "";
}
