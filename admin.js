...
// - Demais tipos: escondido, não faz sentido
function ajustarCamposEvento() {
  ...
  if (tipo === "Gol" || tipo === "Pênalti Marcado") {
    ...
    labelPrincipal.textContent = "Jogador";
...
async function carregarEventosDoJogo() {
  ...
  area.innerHTML = (data || []).map(e => {
    let detalheJogadores = e.jogador_nome || "";
    if (e.tipo === "Substituição" && e.jogador_secundario_nome) {
      detalheJogadores = `${e.jogador_nome || "?"} ⇄ ${e.jogador_secundario_nome}`;
    } else if ((e.tipo === "Gol" || e.tipo === "Pênalti Marcado") && e.jogador_secundario_nome) {
      detalheJogadores = `${e.jogador_nome || "?"} (assist. ${e.jogador_secundario_nome})`;
    }

    return `
    <div class="admin-item">
      <div class="title">${icones[e.tipo] || ""} ${e.minuto}' — ${e.tipo}${detalheJogadores ? " · " + detalheJogadores : ""}</div>
      <div class="actions">
