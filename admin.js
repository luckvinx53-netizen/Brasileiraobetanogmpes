...
let rodadaFiltroAdminSelecionada = "todos";
...
function baixarPdfArbitragemRodada() {
  if (rodadaFiltroAdminSelecionada === "todos") {
...
// Monta os botões de rodada (1ª, 2ª, 3ª...) a partir das rodadas que
// realmente existem entre os jogos cadastrados, mais o botão "Todas".
function montarFiltroRodadaAdmin() {
  const wrap = document.getElementById("filtroRodadaAdminWrap");
  if (!wrap) return;
  ...
  // Se a rodada selecionada não existe mais (jogo excluído etc.), volta pra "todos".
  if (rodadaFiltroAdminSelecionada !== "todos" && !rodadas.includes(Number(rodadaFiltroAdminSelecionada))) {
    rodadaFiltroAdminSelecionada = "todos";
  }

  const botoes = [`<button class="tab-btn${rodadaFiltroAdminSelecionada === "todos" ? " active" : ""}" onclick="selecionarRodadaFiltroAdmin('todos', this)">Todas</button>`]
    .concat(rodadas.map(r => `<button class="tab-btn${String(rodadaFiltroAdminSelecionada) === String(r) ? " active" : ""}" onclick="selecionarRodadaFiltroAdmin(${r}, this)">${r}ª</button>`));

  wrap.innerHTML = botoes.join("");
  atualizarBotaoPdfRodadaAdmin();
}

function selecionarRodadaFiltroAdmin(rodada, btn) {
  rodadaFiltroAdminSelecionada = rodada;
  document.querySelectorAll("#filtroRodadaAdminWrap .tab-btn").forEach(b => b.classList.remove("active"));
  if (btn) btn.classList.add("active");
  atualizarBotaoPdfRodadaAdmin();
  renderizarListaJogosAdmin();
}
...
function atualizarBotaoPdfRodadaAdmin() {
  ...
  if (!botaoPdf) return;
  botaoPdf.style.display = rodadaFiltroAdminSelecionada === "todos" ? "none" : "inline-flex";
}
...
function renderizarListaJogosAdmin() {
  ...
  const jogosFiltrados = rodadaFiltroAdminSelecionada === "todos"
    ? jogosCache
    : jogosCache.filter(j => String(j.rodada) === String(rodadaFiltroAdminSelecionada));
