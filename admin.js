...
function abrirSubAbaJogo(id, btn) {
  ["subDados", "subEscalacao", "subEventos"].forEach(a => {
    document.getElementById(a).classList.add("hidden");
  });
  document.getElementById(id).classList.remove("hidden");
...
async function salvarHoraSimulacao() {
  const id = document.getElementById("jogoId").value;
  if (!id) { notificar("Salve o jogo na aba Dados primeiro.", "aviso"); return; }

  const valor = document.getElementById("hora_inicio_simulacao").value || null;
  const { error } = await supabaseClient.from("jogos").update({ hora_inicio_simulacao: valor }).eq("id", id);
  if (error) { notificar(error.message, "erro"); return; }

  notificar("Horário da simulação salvo!");
  await carregarJogosAdmin();
}
...
async function excluirJogo(id) {
  ...
  const delEscalacoes = await supabaseClient.from("escalacoes_jogo").delete().eq("jogo_id", id);
  if (delEscalacoes.error) console.error("Erro ao excluir escalacoes_jogo:", delEscalacoes.error);
...
async function carregarJogosAdmin() {
  ...
  jogosCache = await Promise.all((data || []).map(j => checarEncerramentoAutomatico(j)));
...
function limparFormularioJogo() {
  ...
  document.getElementById("listaEscalacaoJogo").innerHTML = "";
}
...
async function salvarEscalacao() {
  ...
  if (!jogoId || !timeId || !jogadorNome) {
    notificar("Salve o jogo, e escolha time e jogador.", "aviso");
    return;
  }
