...
function mostrarConteudoAdmin() {
  ...
  // Dashboard visual (KPIs/gráficos do topo) — vive em admin-dashboard.js,
  // só chama se aquele script estiver carregado.
  if (typeof iniciarDashboardAdmin === "function") iniciarDashboardAdmin();
}
...
async function iniciarAdmin() {
  ...
  await carregarEmpresariosParaSelectAdmin();
...
function popularSelectMinutos() {
  ...
  if (!select) return;
  select.innerHTML = "";
  ETAPAS_EVENTO.forEach(etapa => {
    const opt = document.createElement("option");
    opt.value = etapa.valor;
    opt.textContent = etapa.label;
...
function editarTime(t) {
  ...
  document.getElementById("patrocinadorMasterTime").value = t.patrocinador_master || "";
  document.getElementById("patrocinadorMasterLogoTime").value = t.patrocinador_master_logo_url || "";
...
function abrirFormularioNovoTime() {
  ...
  document.getElementById("patrocinadorMasterTime").value = "";
  document.getElementById("patrocinadorMasterLogoTime").value = "";
  document.getElementById("cardFormTime").classList.remove("hidden");
  window.scrollTo({ top: 0, behavior: "smooth" });
}
...
async function salvarTime() {
  ...
  const time = {
    ...
    patrocinador_master: document.getElementById("patrocinadorMasterTime").value.trim() || null,
    patrocinador_master_logo_url: document.getElementById("patrocinadorMasterLogoTime").value.trim() || null,
  };
...
async function encerrarJogoManualmente(id) {
  ...
  // Post automático de "Fim de jogo" no perfil dos dois clubes (antes
  // rodava só no encerramento automático aos 90'; agora o encerramento
  // é sempre manual, então é aqui que precisa disparar). Sem await de
  // propósito — não atrasa a tela esperando o upload da arte pro
  // Storage; se falhar, só loga (ver rsGerarPostFimDeJogoSeNecessario).
...
async function salvarEvento() {
  ...
  const etapaEscolhida = document.getElementById("minutoEvento").value;
  const tipo = document.getElementById("tipoEvento").value;

  if (!jogoId) { notificar("Salve o jogo na aba Dados primeiro.", "aviso"); return; }
  if (!etapaEscolhida) { notificar("Informe a etapa do evento.", "aviso"); return; }
  ...
  const evento = {
    ...
    tipo,
    minuto: etapaParaMinutoReferencia(etapaEscolhida),
  };
  ...
  document.getElementById("minutoEvento").value = "1T";
  document.getElementById("jogadorSecundarioEvento").value = "";
  ...
  area.innerHTML = data.map(e => {
    ...
    return `
      <div class="mc-timeline-item">
        ...
        <div class="mc-timeline-minuto">${minutoParaEtapaLabel(e.minuto)}</div>
...
function editarJogador(j) {
  ...
  document.getElementById("salarioJogador").value = j.salario_mensal ?? "";
  document.getElementById("comissaoJogador").value = j.percentual_comissao ?? "";
  document.getElementById("empresarioJogador").value = j.empresario_id || "";
...
function limparFormularioJogador() {
  ...
  document.getElementById("salarioJogador").value = "";
  document.getElementById("comissaoJogador").value = "";
  document.getElementById("empresarioJogador").value = "";
  document.getElementById("admJogNovoEmpresarioCampos")?.classList.add("hidden");
}

// ---------- EMPRESÁRIOS (compartilhado com o cadastro de jogador) ----------

async function carregarEmpresariosParaSelectAdmin() {
  const select = document.getElementById("empresarioJogador");
  if (!select) return;
  ...
  const opcoesExtras = (data || []).map(e => `<option value="${e.id}">${e.nome}</option>`).join("");
  select.innerHTML = `<option value="">Nenhum</option>${opcoesExtras}<option value="__novo__">+ Cadastrar novo empresário</option>`;
}

// Mostra/esconde os campos de "novo empresário" — reaproveitado pelo
// mesmo padrão de meu-time.js (prefixo diferente pra não colidir ids).
function alternarCampoNovoEmpresario(prefixo) {
  const select = document.getElementById(`${prefixo === "admJog" ? "empresarioJogador" : prefixo + "EmpresarioSelect"}`);
...
async function resolverEmpresarioSelecionadoAdmin() {
  const select = document.getElementById("empresarioJogador");
  if (!select || !select.value) return null;

  if (select.value !== "__novo__") return select.value;

  const nome = document.getElementById("admJogNovoEmpresarioNome").value.trim();
  if (!nome) {
    notificar("Informe o nome do novo empresário.", "aviso");
    return undefined;
  }

  const telefone = document.getElementById("admJogNovoEmpresarioTelefone").value.trim() || null;
  const email = document.getElementById("admJogNovoEmpresarioEmail").value.trim() || null;
...
async function salvarJogador() {
  ...
  const empresarioId = await resolverEmpresarioSelecionadoAdmin();
  if (empresarioId === undefined) return; // erro já notificado

  const salarioInput = document.getElementById("salarioJogador").value;
  const comissaoInput = document.getElementById("comissaoJogador").value;
  ...
  const jogador = {
    ...
    salario_mensal: salarioInput !== "" ? Number(salarioInput) : null,
    percentual_comissao: comissaoInput !== "" ? Number(comissaoInput) : null,
