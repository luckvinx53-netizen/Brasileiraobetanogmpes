...
let mcSubAbaInformacoesAtiva = "arbitragem";
...
async function carregarDetalhes() {
  ...
  mcAbrirSubAbaInformacoes(mcSubAbaInformacoesAtiva);
}
...
// ---------- INFORMAÇÕES (Arbitragem / Estádio) ----------

function mcInformacoesHtml(jogo, arbitragem) {
  return `
    <div class="card">
      <div class="mc-sub-tabs" id="mcSubTabsInformacoes">
        <button class="mc-sub-tab-btn ${mcSubAbaInformacoesAtiva === "arbitragem" ? "active" : ""}"
                onclick="mcAbrirSubAbaInformacoes('arbitragem', this)">🧑‍⚖️ Arbitragem</button>
        <button class="mc-sub-tab-btn ${mcSubAbaInformacoesAtiva === "estadio" ? "active" : ""}"
                onclick="mcAbrirSubAbaInformacoes('estadio', this)">🏟️ Estádio</button>
      </div>

      <div id="mcSubPainelArbitragem" class="mc-sub-panel ${mcSubAbaInformacoesAtiva === "arbitragem" ? "" : "hidden"}">
        ${mcArbitragemHtml(arbitragem)}
      </div>
      <div id="mcSubPainelEstadio" class="mc-sub-panel ${mcSubAbaInformacoesAtiva === "estadio" ? "" : "hidden"}">
        ${mcEstadioHtml(jogo)}
      </div>
    </div>
  `;
...
// Dados do estádio da partida: foto, capacidade, times envolvidos, cidade
// (do time mandante) e data/horário do jogo.
function mcEstadioHtml(jogo) {
  const casa = jogo.time_casa;
  const fora = jogo.time_fora;

  return `
    ${jogo.foto_estadio ? `<img class="mc-estadio-foto" style="margin-top:0;" src="${jogo.foto_estadio}" alt="${jogo.local || "Estádio"}">` : ""}

    <h3 class="mc-estadio-nome">${jogo.local || "Local a definir"}</h3>

    <div class="mc-estadio-detalhes">
      <div class="mc-estadio-item">
        <span class="mc-estadio-label">Times</span>
        <span class="mc-estadio-valor">${casa ? casa.nome : "—"} x ${fora ? fora.nome : "—"}</span>
      </div>
      <div class="mc-estadio-item">
        <span class="mc-estadio-label">Cidade</span>
        <span class="mc-estadio-valor">${casa?.cidade || "—"}</span>
      </div>
      <div class="mc-estadio-item">
        <span class="mc-estadio-label">Data</span>
        <span class="mc-estadio-valor">${formatarData(jogo.data_jogo)}</span>
      </div>
      <div class="mc-estadio-item">
        <span class="mc-estadio-label">Horário</span>
        <span class="mc-estadio-valor">${jogo.hora_jogo || "—"}</span>
      </div>
      ${jogo.capacidade ? `
        <div class="mc-estadio-item">
          <span class="mc-estadio-label">Capacidade</span>
          <span class="mc-estadio-valor">${jogo.capacidade} lugares</span>
        </div>
      ` : ""}
    </div>
  `;
