...
async function carregarDetalhes() {
  ...
  const { minutoAoVivo, statusExibido, emAndamento, temPlacar, pc: placarCasaExibido, pf: placarForaExibido } = estadoAoVivoDoJogo(jogo, gols);
  const jaEncerrado = statusExibido === "Encerrado";
  const aindaNaoComecou = statusExibido === "Agendado";

  const golsCasa = (gols || []).filter(g => {
    if (g.time_id !== casa?.id) return false;
    if (aindaNaoComecou) return false;
    if (emAndamento) return (g.minuto ?? 0) <= minutoAoVivo;
    return true;
  });
  const golsFora = (gols || []).filter(g => {
    if (g.time_id !== fora?.id) return false;
    if (aindaNaoComecou) return false;
    if (emAndamento) return (g.minuto ?? 0) <= minutoAoVivo;
    return true;
  });
