...
function mcExibirBolha(evento, nomeTime) {
  ...
  el.innerHTML = `
    ...
    <span class="mc-evento-bolha-minuto">${minutoParaEtapaLabelCurto(evento.minuto)}</span>
  `;
...
async function carregarDetalhes() {
  ...
  try {
    ...
    // checarEncerramentoAutomatico não encerra mais o jogo sozinha (ver
    // utils.js) — o encerramento agora é sempre manual, via botão no
    // admin. A chamada continua aqui só pelo backfill do post de "Fim
    // de jogo" em jogos que já estavam Encerrados antes dessa mudança.
    // Se algo falhar aqui, seguimos com os dados originais do jogo em
...
function mcHeroHtml(jogo, estado) {
  ...
  const { statusExibido, emAndamento, temPlacar, pc, pf } = estado;

  const subinfo = emAndamento
    ? `<span class="minuto-live">Em andamento</span>`
    ...
    <div class="mc-hero">
      <div class="mc-hero-top">
        ...
        <span class="status-pill ${statusClasse(statusExibido)}">
          ${emAndamento ? `<span class="mc-status-live"><span class="dot"></span>AO VIVO</span>` : statusExibido}
        </span>
      </div>
...
function mcTimelineLinhaHtml(e, casa, fora) {
  ...
  if (EVENTOS_MARCO.has(e.tipo)) {
    return `<div class="mc-timeline-marco">${icone} ${e.tipo} — ${minutoParaEtapaLabelCurto(e.minuto)}</div>`;
  }
  ...
  return `
    <div class="mc-timeline-item" data-evento-id="${e.id}">
      ...
      <div class="mc-timeline-minuto">${minutoParaEtapaLabelCurto(e.minuto)}</div>
