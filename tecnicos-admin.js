...
async function vincularTecnico() {
  const uuid = document.getElementById("uuidTecnico").value.trim();
  const timeId = document.getElementById("timeTecnicoNovo").value;
  const nome = document.getElementById("nomeTecnicoNovo").value.trim();

  if (!uuid || !timeId) {
    notificar("Informe o UUID do usuário e escolha o time.", "aviso");
    return;
  }

  const { error } = await supabaseClient
    .from("tecnicos")
    .upsert({ user_id: uuid, time_id: timeId, nome }, { onConflict: "user_id" });

  if (error) {
    notificar("Erro ao vincular: " + error.message, "erro");
    return;
  }

  notificar("Técnico vinculado com sucesso!");
  document.getElementById("uuidTecnico").value = "";
