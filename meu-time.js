...
function abrirModalConsultaValor(jogadorId, timeDonoId, ehExterno) {
  const overlay = document.createElement("div");
  overlay.className = "modal-jogador-overlay";
...
function abrirModalContrato(consultaId) {
  ...
  overlay.innerHTML = `
    <div class="modal-jogador-sheet">
      <h3>Propor contrato a ${nomeJogador}</h3>
      <p class="text-dim" style="font-size:12.5px;">
        O clube já aceitou a transferência. Agora negocie diretamente as condições contratuais com o jogador.
      </p>
      <div class="field">
        <label>Salário mensal</label>
        <input type="number" id="salarioContratoInput" placeholder="Ex: 350000" min="0" step="1">
      </div>
      <div class="field">
        <label>Duração do contrato (meses)</label>
        <input type="number" id="duracaoContratoInput" placeholder="Ex: 36" min="1" step="1">
      </div>
      <div class="field">
        <label>Bônus de assinatura (opcional)</label>
        <input type="number" id="bonusAssinaturaContratoInput" placeholder="Ex: 200000" min="0" step="1">
      </div>
      <div class="field">
        <label>Condição do bônus (opcional)</label>
        <input type="text" id="bonusCondicaoContratoInput" placeholder="Ex: R$ 50.000 a cada 10 gols na temporada">
      </div>
      <div class="field">
        <label>Observações (opcional)</label>
        <input type="text" id="obsContratoInput" placeholder="Outras condições combinadas">
      </div>
      <button class="btn btn-primary btn-block" onclick="enviarContratoNegociacao('${consultaId}')">Enviar proposta de contrato</button>
      <button class="btn btn-ghost btn-block" style="margin-top:10px;" onclick="document.getElementById('modalContratoOverlay').remove()">Cancelar</button>
    </div>
  `;
...
async function enviarContratoNegociacao(consultaId) {
  ...
  const duracao = document.getElementById("duracaoContratoInput").value;
  const bonus = document.getElementById("bonusAssinaturaContratoInput").value;
  const bonusCondicao = document.getElementById("bonusCondicaoContratoInput").value.trim();
  const obs = document.getElementById("obsContratoInput").value.trim();
  ...
  if (!duracao || Number(duracao) <= 0) {
    notificar("Informe a duração do contrato em meses.", "aviso");
    return;
  }
...
async function consultarEmpresarioSobreContrato(transferenciaId, contrato) {
  const { data: transferencia } = await supabaseClient
    .from("bid_transferencias")
    .select("*, jogadores(nome, posicao, overall, salario_mensal, valor_mercado, empresario_id, percentual_comissao, empresario:empresario_id(nome)), jogador_externo:jogador_externo_id(nome, sobrenome, posicao, overall, salario_mensal, valor_mercado, empresario_id, percentual_comissao, empresario:empresario_id(nome)), interessado:time_interessado_id(nome)")
...
async function aceitarContratoNegociacao(contratoId, transferenciaId) {
  ...
  // Aplica o salário negociado e move o jogador — só para jogador do
  // jogo. Jogador externo fica de fora do elenco por enquanto (ver
  // comentário em jogadores_externos no banco).
  if (transferencia.jogador_id) {
    const { error: erroJogador } = await supabaseClient
      ...
      .update({
        ...
        regularizado: false,
        salario_mensal: contrato.salario_mensal_proposto,
      })
      .eq("id", transferencia.jogador_id);
  ...
  // orçamento movimentado, comissão descontada). É só aqui que
  // status volta a valer "aceito" — o mesmo valor que todas as outras
  // telas do site (Regularização, BID público, notícias de mercado,
  // perfil do jogador, Transfermarkt) já esperam para considerar uma
  // transferência como realmente concluída.
  await supabaseClient
    .from("bid_transferencias")
...
async function carregarConsultasEnviadas() {
  ...
  const { data, error } = await supabaseClient
    .from("bid_transferencias")
    .select("*, jogadores(nome), jogador_externo:jogador_externo_id(nome, sobrenome), dono:time_dono_id(nome)")
    .eq("time_interessado_id", meuTimeId)
    .order("criado_em", { ascending: false });
  ...
  lista.innerHTML = data.map(c => {
    ...
    <div class="consulta-item">
      <div class="info">
        ...
        ${scoutDetalhesPropostaHtml(c)}
        ${etapaTransferenciaHtml(c, contrato)}
        ${contrato ? `<p class="text-dim" style="font-size:12px;margin-top:4px;">Contrato proposto: R$ ${Number(contrato.salario_mensal_proposto).toLocaleString("pt-BR")}/mês por ${contrato.duracao_meses} meses${contrato.bonus_assinatura > 0 ? ` + bônus de R$ ${Number(contrato.bonus_assinatura).toLocaleString("pt-BR")}` : ""}</p>` : ""}
        ${contrato?.observacoes ? `<p class="text-dim" style="font-size:12px;margin-top:4px;font-style:italic;">${contrato.observacoes}</p>` : ""}
      </div>
      ${c.status === "negociando" && c.proposta_de !== "interessado" ? `
        <div class="flex-gap" style="margin-top:10px;">
      ...
      ${contrato?.status === "pendente" ? `
        <div class="flex-gap" style="margin-top:10px;">
          <button class="btn btn-primary btn-sm" onclick="aceitarContratoNegociacao('${contrato.id}', '${c.id}')">Aceitar mesmo assim — concluir</button>
          <button class="btn btn-secondary btn-sm" onclick="abrirModalContrato('${c.id}')">Enviar nova proposta</button>
          <button class="btn btn-ghost btn-sm" onclick="recusarContratoNegociacao('${contrato.id}')">Desistir da negociação</button>
        </div>
        ${!contrato.observacoes ? `<p class="text-dim" style="font-size:11.5px;margin-top:6px;">Sem empresário configurado — como não há login de jogador no sistema, quem registra a resposta é você mesmo, técnico do clube comprador.</p>` : ""}
      ` : ""}
    </div>
  `;
  }).join("");
  ...
  lista.innerHTML = data.map(j => {
    ...
    <div class="time-item">
      ...
      ${j.status === "disponivel" && !jaNegociando ? `
        <div class="flex-gap" style="margin-top:10px;">
          <button class="btn btn-primary btn-sm" onclick="abrirModalConsultaValor('${j.id}', '${j.clube_atual_id || ""}', true)">Enviar proposta</button>
        </div>
      ` : ""}
