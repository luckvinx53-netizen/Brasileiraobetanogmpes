...
function renderizarCabecalho() {
  ...
  if (typeof aplicarTemaTime === "function") aplicarTemaTime(t.nome);
}
...
async function verificarJogoParaEscalar() {
  ...
  // Acha o próximo jogo cuja janela de prazo já esteja aberta
  const agora = new Date();
  let jogoEncontrado = null;

  for (const jogo of jogos) {
    if (!jogo.data_jogo) continue;
    const dataHoraJogo = new Date(`${jogo.data_jogo}T${jogo.hora_jogo || "00:00"}:00`);
    const abreEm = new Date(dataHoraJogo.getTime() - horasAntes * 60 * 60 * 1000);

    if (agora >= abreEm && agora < dataHoraJogo) {
      jogoEncontrado = jogo;
      break;
    }
  }
  ...
  jogoParaEscalar = jogoEncontrado;
  bloco.classList.remove("hidden");

  const adversario = jogoEncontrado.time_casa_id === meuTimeId
    ? jogoEncontrado.time_fora?.nome
    : jogoEncontrado.time_casa?.nome;

  const dataHoraJogo = new Date(`${jogoEncontrado.data_jogo}T${jogoEncontrado.hora_jogo || "00:00"}:00`);
  const horasRestantes = (dataHoraJogo - agora) / 1000 / 60 / 60;
  const urgente = horasRestantes < 1;

  document.getElementById("infoJogoEscalacao").innerHTML = `
    <span class="prazo-badge ${urgente ? 'urgente' : ''}">⏰ Prazo fecha em breve</span>
    <p style="margin:0;font-size:14px;">Próximo jogo: <strong>${jogoEncontrado.rodada}ª rodada vs ${adversario}</strong></p>
  `;
