...
function abrirModalContrapropostaPatrocinio(patrocinioId) {
  ...
  overlay.innerHTML = `
    <div class="negociacao-topo">
      <h3>Contraproposta de patrocínio</h3>
    ...
    <div class="negociacao-acoes">
      <button class="btn btn-primary btn-block" onclick="enviarContrapropostaPatrocinio('${p.id}')">Enviar contraproposta</button>
...
async function enviarContrapropostaPatrocinio(patrocinioId) {
  ...
  try {
    ...
    if (resultado.decisao === "aceito") {
      ...
      await supabaseClient
        .from("patrocinios")
        .update({ status: "recusado", ultima_fala: resultado.fala, respondido_em: new Date().toISOString() })
        .eq("id", patrocinioId);

      notificar(`${p.empresa_nome} recusou. "${resultado.fala}"`, "aviso");
