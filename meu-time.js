...
function renderizarCabecalho() {
  ...
  area.innerHTML = `
    <div class="card cabecalho-time-tema" style="display:flex;align-items:center;">
      ${escudoHtml(t, "escudo")}
      <div>
        <span class="eyebrow-tecnico">Área do técnico</span>
        <h1>${t.nome}</h1>
      </div>
    </div>
  `;
...
async function carregarElenco() {
  ...
  lista.innerHTML = elencoDoTime.map(j => `
    <div class="elenco-card" onclick="location.href='jogador?id=${j.id}'">
      <div class="elenco-card-numero">${j.numero ?? "-"}</div>
      <div class="info">
        <h3>${j.nome} ${j.regularizado === false ? '<span class="jogador-irregular">· Irregular (BID)</span>' : ""}</h3>
        <p>${j.posicao || "—"} <span class="elenco-card-dot"></span> ${j.gols} gols <span class="elenco-card-dot"></span> ${j.assistencias} assist.</p>
      </div>
      <span class="elenco-card-seta">›</span>
    </div>
  `).join("");
}
...
// Cache das propostas/negociações de patrocínio do time atual,
// indexado por id — usado pela tela de negociação de patrocínio
// (abrirNegociacaoPatrocinio), no mesmo espírito da de contrato.
let patrociniosCache = {};
...
async function carregarOrcamentoMeuTime() {
  ...
  const categoriasConfig = [
    ...
    { motivo: "patrocinio_master", tipo: "entrada", label: "🤝 Patrocínio master", cor: "#2dd4bf" },
    { motivo: "patrocinio_manga", tipo: "entrada", label: "🤝 Patrocínio de manga", cor: "#2dd4bf" },
    { motivo: "patrocinio_costas", tipo: "entrada", label: "🤝 Patrocínio de costas", cor: "#2dd4bf" },
    { motivo: "patrocinio_calcao", tipo: "entrada", label: "🤝 Patrocínio de calção", cor: "#2dd4bf" },
    { motivo: "multa_rescisao_patrocinio", tipo: "saida", label: "⛔ Multas de rescisão", cor: "#e5484d" },
  ...
  const motivoLabel = {
    ...
    patrocinio_manga: "🤝 Patrocínio de manga",
    patrocinio_costas: "🤝 Patrocínio de costas",
    patrocinio_calcao: "🤝 Patrocínio de calção",
    multa_rescisao_patrocinio: "⛔ Multa de rescisão de patrocínio",
  };
...
// ---------------------------------------------------------
// PATROCÍNIOS
// Propostas chegam sozinhas (sorteio diário no banco + IA via Gemini),
// mandadas por empresas reais (Superbet, Ademicon, etc) pros 4 slots
// possíveis: master (peito), manga, costas, calção. A tela de
// negociação segue o mesmo padrão visual da negociação de contrato
// de jogador — só que aqui é o clube negociando com o representante
// da empresa, e aceitar credita o valor mensal automaticamente todo
// mês (via cron), sem mexer no elenco.
// ---------------------------------------------------------

const SLOT_PATROCINIO_LABEL = {
  master: "Patrocinador master (peito)",
  manga: "Patrocinador de manga",
  costas: "Patrocinador de costas",
  calcao: "Patrocinador de calção",
};
const SLOT_PATROCINIO_ICONE = { master: "🎽", manga: "💪", costas: "🔙", calcao: "🩳" };
const SLOTS_PATROCINIO = ["master", "manga", "costas", "calcao"];

async function carregarPatrocinios() {
  const lista = document.getElementById("listaPatrocinios");
  if (!lista) return;
  ...
  if (error) {
    lista.innerHTML = `<p class="text-dim" style="font-size:13px;">Erro ao carregar patrocínios.</p>`;
    return;
  }

  patrociniosCache = {};
  (data || []).forEach(p => { patrociniosCache[p.id] = p; });

  // Pra cada slot, pega o patrocínio mais recente relevante (aceito
  // tem prioridade; senão, uma proposta pendente; senão, vazio).
  lista.innerHTML = SLOTS_PATROCINIO.map(slot => {
    const doSlot = (data || []).filter(p => p.slot === slot);
    const aceito = doSlot.find(p => p.status === "aceito");
    const pendente = doSlot.find(p => p.status === "pendente" || p.status === "gerando");
    const atual = aceito || pendente;

    if (!atual) {
      return `
        <div class="time-item">
          <span style="font-size:22px;">${SLOT_PATROCINIO_ICONE[slot]}</span>
          <div class="info">
            <h3 style="font-size:14px;">${SLOT_PATROCINIO_LABEL[slot]}</h3>
            <p>Nenhuma proposta no momento</p>
          </div>
        </div>
      `;
    }
    ...
    if (atual.status === "gerando") {
      return `
        <div class="time-item">
          ...
          <div class="info">
            <h3 style="font-size:14px;">${SLOT_PATROCINIO_LABEL[slot]}</h3>
            <p>${atual.empresa_nome} está preparando uma proposta...</p>
          </div>
        </div>
      `;
    }

    const statusLabel = atual.status === "aceito"
      ? `<span style="color:var(--grama);">Ativo — R$ ${Number(atual.valor_mensal).toLocaleString("pt-BR")}/mês (${atual.meses_creditados}/${atual.duracao_meses} meses)</span>`
      : `<span style="color:#e6c869;">Proposta pendente — R$ ${Number(atual.valor_mensal).toLocaleString("pt-BR")}/mês</span>`;

    return `
      <div class="time-item" style="cursor:pointer;" onclick="abrirNegociacaoPatrocinio('${atual.id}')">
        <span style="font-size:22px;">${SLOT_PATROCINIO_ICONE[slot]}</span>
        <div class="info">
          <h3 style="font-size:14px;">${SLOT_PATROCINIO_LABEL[slot]} — ${atual.empresa_nome}</h3>
          <p>${statusLabel}</p>
        </div>
      </div>
    `;
...
// Tela de negociação em tela cheia, no mesmo espírito visual da
// negociação de contrato de jogador (abrirTelaNegociacao): mostra a
// "ligação" com o representante da empresa, os termos da proposta
// atual e as ações disponíveis.
function abrirNegociacaoPatrocinio(patrocinioId) {
  const p = patrociniosCache?.[patrocinioId];
  if (!p) {
    notificar("Não foi possível abrir a negociação.", "erro");
    return;
  }
  ...
  const overlay = document.createElement("div");
  overlay.className = "negociacao-overlay";
  overlay.id = "telaNegociacaoPatrocinioOverlay";

  const statusTag = p.status === "aceito" ? "Patrocínio ativo"
    : p.status === "recusado" ? "Recusado"
    : "Em negociação";

  overlay.innerHTML = `
    <div class="negociacao-topo">
      <h3>Negociação de patrocínio</h3>
      <button class="fechar" onclick="document.getElementById('telaNegociacaoPatrocinioOverlay').remove()">✕</button>
    </div>

    <div class="negociacao-call-card">
      <div class="avatar">${p.empresa_nome.slice(0, 1).toUpperCase()}</div>
      <div class="info">
        <span class="tag-status">${statusTag}</span>
        <h4>${p.empresa_nome}</h4>
        <p>${p.empresa_segmento || "Empresa patrocinadora"}</p>
      </div>
    </div>

    ${p.ultima_fala ? `<p class="negociacao-fala">💬 "${p.ultima_fala}"</p>` : ""}

    <div class="negociacao-jogador-card">
      <div class="cabecalho">
        <h4>${SLOT_PATROCINIO_LABEL[p.slot]}</h4>
      </div>
    </div>
    ...
    <div class="negociacao-proposta">
      <div class="linha destaque">
        ...
        <span class="valor">R$ ${Number(p.valor_mensal).toLocaleString("pt-BR")}</span>
      </div>
      <div class="linha">
        <span class="label">Duração</span>
        <span class="valor">${formatarDuracaoContrato(p.duracao_meses)}</span>
      </div>
      ${p.status === "aceito" ? `
      <div class="linha">
        <span class="label">Tempo restante de contrato</span>
        <span class="valor" style="font-weight:400;font-size:12px;">${calcularTempoRestanteContrato(p.aceito_em, p.duracao_meses)}</span>
      </div>
      <div class="linha">
        <span class="label">Multa de rescisão</span>
        <span class="valor" style="font-weight:400;font-size:12px;">R$ ${Number(p.multa_rescisao || Math.round(p.valor_mensal * 3)).toLocaleString("pt-BR")}</span>
      </div>
      ` : ""}
      <div class="linha">
        <span class="label">Bônus de desempenho</span>
        <span class="valor">${p.bonus_valor > 0 ? `R$ ${Number(p.bonus_valor).toLocaleString("pt-BR")}` : "Nenhum"}</span>
      </div>
      ${p.bonus_condicao ? `
      <div class="linha">
        <span class="label">Condição do bônus</span>
        <span class="valor" style="font-weight:400;font-size:12px;">${{gol:"Por gol marcado",vitoria:"Por vitória",titulo:"Por título"}[p.bonus_condicao] || p.bonus_condicao}</span>
      </div>` : ""}
    </div>

    <div class="negociacao-acoes">
      ${p.status === "pendente" ? `
        <button class="btn btn-primary btn-block" onclick="aceitarPatrocinio('${p.id}')">Aceitar proposta</button>
        <button class="btn btn-secondary btn-block" onclick="document.getElementById('telaNegociacaoPatrocinioOverlay').remove(); abrirModalContrapropostaPatrocinio('${p.id}');">Enviar contraproposta</button>
        <button class="btn btn-ghost btn-block" onclick="document.getElementById('telaNegociacaoPatrocinioOverlay').remove(); recusarPatrocinio('${p.id}');">Recusar</button>
      ` : p.status === "aceito" ? `
        <button class="btn btn-primary btn-block" onclick="abrirRenovacaoPatrocinio('${p.id}')">Renovar/renegociar</button>
        <button class="btn btn-ghost btn-block" style="color:#e5484d;" onclick="confirmarRescisaoPatrocinio('${p.id}')">Rescindir contrato</button>
      ` : `
        <button class="btn btn-ghost btn-block" onclick="document.getElementById('telaNegociacaoPatrocinioOverlay').remove()">Fechar</button>
      `}
    </div>
  `;
...
// Calcula quanto tempo falta de um contrato de patrocínio, a partir
// da data de aceite e da duração combinada — usado só para exibição
// (o crédito mensal em si é controlado por meses_creditados, à parte).
function calcularTempoRestanteContrato(aceitoEm, duracaoMeses) {
  if (!aceitoEm || !duracaoMeses) return "—";
  const fim = new Date(aceitoEm);
  fim.setMonth(fim.getMonth() + Number(duracaoMeses));
  const hoje = new Date();
  const mesesRestantes = Math.max(0, Math.round((fim - hoje) / (1000 * 60 * 60 * 24 * 30)));
  if (mesesRestantes === 0) return "Contrato encerrado";
  return formatarDuracaoContrato(mesesRestantes) + " restantes";
}

// Abre o modal pro técnico enviar uma contraproposta (novo valor/duração)
// ao invés de aceitar ou recusar de cara.
function abrirModalContrapropostaPatrocinio(patrocinioId) {
  const p = patrociniosCache?.[patrocinioId];
  if (!p) return;

  const overlay = document.createElement("div");
  overlay.className = "negociacao-overlay";
  overlay.id = "modalContrapropostaPatrocinioOverlay";

  const ehRenovacao = p.status === "aceito";
  const PASSO_VALOR = 5000;

  overlay.innerHTML = `
    <div class="negociacao-topo">
      <h3>${ehRenovacao ? "Renovar patrocínio" : "Contraproposta de patrocínio"}</h3>
      <button class="fechar" onclick="document.getElementById('modalContrapropostaPatrocinioOverlay').remove()">✕</button>
    </div>
    <div class="negociacao-form">
      <h2 class="negociacao-form-titulo">${p.empresa_nome} — ${SLOT_PATROCINIO_LABEL[p.slot]}</h2>
      <div class="field">
        <label>Valor mensal desejado</label>
        <div class="value-stepper">
          <button type="button" onclick="ajustarValorStepper('valorContrapropostaPatrocinioInput', -${PASSO_VALOR}, 0)">−</button>
          <input type="number" id="valorContrapropostaPatrocinioInput" value="${Number(p.valor_mensal) || 0}" min="0" step="${PASSO_VALOR}" inputmode="numeric">
          <button type="button" onclick="ajustarValorStepper('valorContrapropostaPatrocinioInput', ${PASSO_VALOR}, 0)">+</button>
        </div>
      </div>
      <div class="field">
        <label>Duração do contrato</label>
        <select id="duracaoContrapropostaPatrocinioInput">
          ${Array.from({ length: 6 }, (_, i) => i + 1).map(ano => `<option value="${ano}" ${ano * 12 === Number(p.duracao_meses) ? "selected" : ""}>${ano} ${ano === 1 ? "ano" : "anos"}</option>`).join("")}
        </select>
      </div>
    </div>
    <div class="negociacao-acoes">
      <button class="btn btn-primary btn-block" onclick="enviarContrapropostaPatrocinio('${p.id}')">${ehRenovacao ? "Enviar proposta de renovação" : "Enviar contraproposta"}</button>
      <button class="btn btn-ghost btn-block" onclick="document.getElementById('modalContrapropostaPatrocinioOverlay').remove()">Cancelar</button>
    </div>
  `;
...
async function enviarContrapropostaPatrocinio(patrocinioId) {
  const p = patrociniosCache?.[patrocinioId];
  if (!p) return;

  const valorInput = document.getElementById("valorContrapropostaPatrocinioInput").value;
  const anosInput = document.getElementById("duracaoContrapropostaPatrocinioInput").value;

  if (!valorInput || Number(valorInput) <= 0) {
    notificar("Informe o valor mensal desejado.", "aviso");
    return;
  }

  const valorAnterior = Number(p.valor_mensal);
  const duracaoAnterior = Number(p.duracao_meses);
  const valorProposto = Number(valorInput);
  const duracaoProposta = Number(anosInput) * 12;

  document.getElementById("modalContrapropostaPatrocinioOverlay")?.remove();

  // Conta quantas rodadas de negociação já rolaram (aproximado, pelo
  // número de vezes que ultima_fala já foi preenchida) pra dar mais
  // flexibilidade à empresa em tentativas seguintes — mesmo espírito
  // do tentativaNumero em consultarEmpresarioSobreContrato.
  const tentativaNumero = p.respondido_em ? 2 : 1;

  try {
    const { data: { session } } = await supabaseClient.auth.getSession();
    const { data: time } = await supabaseClient.from("times").select("nome").eq("id", meuTimeId).single();

    const resposta = await fetch(`${SUPABASE_URL}/functions/v1/simular-patrocinio`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${session?.access_token || ""}`,
      ...
      body: JSON.stringify({
        acao: "responder_negociacao",
        empresaNome: p.empresa_nome,
        empresaSegmento: p.empresa_segmento,
        slot: p.slot,
        valorMensalAnterior: valorAnterior,
        duracaoMesesAnterior: duracaoAnterior,
        valorMensalProposto: valorProposto,
        duracaoMesesProposta: duracaoProposta,
        nomeTime: time?.nome,
        tentativaNumero,
      }),
    });
    ...
    if (!resposta.ok || resultado.error) {
      notificar("Não foi possível consultar a empresa agora. Tente de novo em instantes.", "erro");
      return;
    }
    ...
    if (resultado.decisao === "aceito") {
      await supabaseClient
        .from("patrocinios")
        .update({
          valor_mensal: valorProposto,
          duracao_meses: duracaoProposta,
          ultima_fala: resultado.fala,
          respondido_em: new Date().toISOString(),
        })
        .eq("id", patrocinioId);

      notificar(`${p.empresa_nome} aceitou! "${resultado.fala}"`);
      await aceitarPatrocinio(patrocinioId, { jaAtualizouTermos: true });
    } else if (resultado.decisao === "recusado") {
      // Se era uma renovação (patrocínio já estava "aceito"), recusar
      // a renovação NÃO encerra o contrato vigente — só volta a ser
      // "aceito" com os termos antigos intactos, sem tocar em
      // valor_mensal/duracao_meses/aceito_em/multa_rescisao.
      const eraRenovacao = p.status === "aceito";
      await supabaseClient
        .from("patrocinios")
        .update({
          status: eraRenovacao ? "aceito" : "recusado",
          ultima_fala: resultado.fala,
          respondido_em: new Date().toISOString(),
        })
        .eq("id", patrocinioId);
      ...
      notificar(
        eraRenovacao
          ? `${p.empresa_nome} não topou renovar nesses termos. O contrato atual continua valendo. "${resultado.fala}"`
          : `${p.empresa_nome} recusou. "${resultado.fala}"`,
        "aviso"
      );
      await carregarPatrocinios();
    } else {
      // contraproposta da empresa: atualiza os termos exibidos com a
      // sugestão dela, mantém pendente pro clube decidir o próximo passo.
      await supabaseClient
        .from("patrocinios")
        .update({
          valor_mensal: resultado.contraproposta_valor_mensal || valorAnterior,
          duracao_meses: resultado.contraproposta_duracao_meses || duracaoAnterior,
          ultima_fala: resultado.fala,
          respondido_em: new Date().toISOString(),
        })
        .eq("id", patrocinioId);

      notificar(`${p.empresa_nome} quer negociar. "${resultado.fala}"`, "aviso");
      await carregarPatrocinios();
    }
  } catch (e) {
    console.error("Falha ao negociar patrocínio:", e);
    notificar("Não foi possível consultar a empresa agora.", "erro");
  }
}

// Aceitar = ativa o patrocínio de fato (status "aceito", aceito_em
// preenchido). O crédito mensal no orçamento é automático a partir
// daí, via creditar_patrocinios_do_mes() (cron todo dia 1º).
async function aceitarPatrocinio(patrocinioId, opts) {
  const p = patrociniosCache?.[patrocinioId];
  const multa = p ? Math.round(Number(p.valor_mensal) * 3) : null;

  const { error } = await supabaseClient
    .from("patrocinios")
    .update({ status: "aceito", aceito_em: new Date().toISOString(), multa_rescisao: multa })
    .eq("id", patrocinioId);

  document.getElementById("telaNegociacaoPatrocinioOverlay")?.remove();

  if (error) {
    notificar("Erro ao aceitar patrocínio: " + error.message, "erro");
    return;
  }

  if (!opts?.jaAtualizouTermos) {
    notificar("Patrocínio aceito! O valor mensal passa a entrar no orçamento automaticamente todo mês.");
  }
  await carregarPatrocinios();
  await carregarEmailMeuTime();
}

async function recusarPatrocinio(patrocinioId) {
  const { error } = await supabaseClient
    .from("patrocinios")
    .update({ status: "recusado", respondido_em: new Date().toISOString() })
    .eq("id", patrocinioId);
  ...
  if (error) {
    notificar("Erro ao recusar patrocínio: " + error.message, "erro");
    return;
  }

  notificar("Proposta de patrocínio recusada.");
  await carregarPatrocinios();
  await carregarEmailMeuTime();
}

// Rescindir um patrocínio ATIVO antes do fim do contrato: cobra multa
// fixa de 3 meses do valor mensal (calculada no momento em que o
// patrocínio foi aceito) e libera o slot pra novas propostas. O valor
// da multa sai do orçamento do clube como uma movimentação normal.
function confirmarRescisaoPatrocinio(patrocinioId) {
  const p = patrociniosCache?.[patrocinioId];
  if (!p) return;

  const multa = Number(p.multa_rescisao) || Math.round(Number(p.valor_mensal) * 3);

  if (!confirm(`Rescindir o patrocínio com ${p.empresa_nome}? Isso custa uma multa de R$ ${multa.toLocaleString("pt-BR")} (3 meses de contrato), descontada do orçamento agora. O slot fica livre pra novas propostas.`)) {
    return;
  }
  executarRescisaoPatrocinio(patrocinioId, multa);
}

async function executarRescisaoPatrocinio(patrocinioId, multa) {
  const { error: erroPatrocinio } = await supabaseClient
    .from("patrocinios")
    .update({ status: "rescindido", rescindido_em: new Date().toISOString() })
    .eq("id", patrocinioId);

  if (erroPatrocinio) {
    notificar("Erro ao rescindir patrocínio: " + erroPatrocinio.message, "erro");
    return;
  }

  // Debita a multa do orçamento do time, como qualquer outra movimentação.
  const { data: time } = await supabaseClient.from("times").select("orcamento").eq("id", meuTimeId).single();
  const novoOrcamento = Number(time?.orcamento || 0) - multa;

  await supabaseClient.from("times").update({ orcamento: novoOrcamento }).eq("id", meuTimeId);
  await supabaseClient.from("orcamento_movimentacoes").insert([{
    time_id: meuTimeId, tipo: "saida", motivo: "multa_rescisao_patrocinio", valor: multa,
  }]);

  document.getElementById("telaNegociacaoPatrocinioOverlay")?.remove();
  notificar(`Patrocínio rescindido. Multa de R$ ${multa.toLocaleString("pt-BR")} debitada do orçamento.`);
  await carregarPatrocinios();
  await carregarOrcamentoMeuTime();
}

// Renovar um patrocínio ativo: mantém a mesma empresa/slot, e permite
// ajustar valor e duração antes de confirmar — reaproveita o mesmo
// modal de contraproposta, só que a partir de um patrocínio já aceito
// (em vez de uma proposta pendente).
function abrirRenovacaoPatrocinio(patrocinioId) {
  document.getElementById("telaNegociacaoPatrocinioOverlay")?.remove();
  abrirModalContrapropostaPatrocinio(patrocinioId);
}
...
async function carregarEmailMeuTime() {
  ...
  // Propostas de patrocínio também aparecem no Email — a linha
  // "gerando" (IA ainda escrevendo a proposta) não entra aqui.
  const { data: patrocinios, error: erroPatrocinios } = await supabaseClient
    .from("patrocinios")
    .select("*")
    .eq("time_id", meuTimeId)
    .neq("status", "gerando")
  ...
  if (error || erroPatrocinios) {
    lista.innerHTML = `<div class="empty-state"><div class="icon">⚠️</div><h3>Erro ao carregar mensagens</h3></div>`;
    return;
  }

  const itensTransferencia = (data || []).map(c => ({ tipo: "transferencia", data: c.criado_em, item: c }));
  const itensPatrocinio = (patrocinios || []).map(p => ({ tipo: "patrocinio", data: p.criado_em, item: p }));
  const todosItens = [...itensTransferencia, ...itensPatrocinio].sort((a, b) => new Date(b.data) - new Date(a.data));
  ...
  if (todosItens.length === 0) {
    lista.innerHTML = `
      <div class="empty-state">
        ...
        <p>Avisos sobre consultas de transferência, propostas de patrocínio e outras notificações do time aparecem aqui.</p>
      </div>
    `;
    return;
  }
  ...
  const tipoLabel = { definitivo: "Definitivo", emprestimo: "Empréstimo" };
  const statusPatrocinioLabel = { pendente: "Aguardando você negociar", recusado: "Recusado", aceito: "Patrocínio ativo" };

  lista.innerHTML = todosItens.map(({ tipo, item: c }) => {
    if (tipo === "patrocinio") {
      return `
        <div class="email-item ${c.lida ? "" : "nao-lida"}">
          <div class="email-topo">
            <h4 class="email-titulo">🤝 Proposta de patrocínio — ${c.empresa_nome}</h4>
            <span class="email-data">${new Date(c.criado_em).toLocaleString("pt-BR")}</span>
          </div>
          <p class="email-corpo">
            A <strong>${c.empresa_nome}</strong> (${c.empresa_segmento || "empresa"}) quer patrocinar o ${SLOT_PATROCINIO_LABEL[c.slot]?.toLowerCase() || c.slot} do seu time.
            Proposta: R$ ${Number(c.valor_mensal).toLocaleString("pt-BR")}/mês por ${formatarDuracaoContrato(c.duracao_meses)}.
            ${c.ultima_fala ? ` "${c.ultima_fala}"` : ""}
            <br><span class="status-consulta ${c.status}">${statusPatrocinioLabel[c.status] || c.status}</span>
          </p>
          <div class="email-acoes">
            ${c.status === "pendente" ? `<button class="btn btn-primary btn-sm" onclick="abrirNegociacaoPatrocinioDoEmail('${c.id}')">Ir para negociação</button>` : ""}
          </div>
        </div>
      `;
    }
  ...
  const idsPatrociniosNaoLidos = (patrocinios || []).filter(p => !p.lida).map(p => p.id);
  if (idsPatrociniosNaoLidos.length > 0) {
    await supabaseClient.from("patrocinios").update({ lida: true }).in("id", idsPatrociniosNaoLidos);
  }
  if (idsNaoLidas.length > 0 || idsPatrociniosNaoLidos.length > 0) {
    await atualizarBolinhaEmail();
  }
}

// Botão "Ir para negociação" dentro do email de patrocínio: leva pra
// aba Orçamento e abre a tela de negociação diretamente, já que a
// negociação de patrocínio mora lá (não na aba BID).
function abrirNegociacaoPatrocinioDoEmail(patrocinioId) {
  const linkOrcamento = document.querySelector('#navMeuTime a[data-secao="Orcamento"]');
  abrirSecaoMeuTime("Orcamento", linkOrcamento);
  setTimeout(() => abrirNegociacaoPatrocinio(patrocinioId), 150);
}
