...
async function limparHistoricoBid(escopo) {
  const confirmou = confirm(
    "Isso vai apagar permanentemente as consultas/propostas já encerradas (recusadas ou concluídas) dessa lista. Negociações em andamento não são afetadas. Deseja continuar?"
  );
  if (!confirmou) return;
  ...
  notificar(count ? `${count} registro(s) apagado(s) do histórico.` : "Nada para apagar — histórico já estava limpo.");
...
// respeitando o mínimo (e nunca deixando o campo vazio/NaN).
function ajustarValorStepper(inputId, passo, minimo) {
  const input = document.getElementById(inputId);
  if (!input) return;
  const atual = Number(input.value) || 0;
  const novo = Math.max(minimo, atual + passo);
  input.value = novo;
}
