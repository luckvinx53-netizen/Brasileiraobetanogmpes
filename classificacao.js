...
const CLASS_FASES_MATA_MATA = ["oitavas", "quartas", "semifinal", "final"];
const CLASS_NOME_FASE_MATA_MATA = {
  oitavas: "Oitavas",
  quartas: "Quartas",
  semifinal: "Semifinal",
  final: "Final",
...
async function carregarBracketNaClassificacao(area) {
  ...
  try {
    ...
    area.innerHTML = `<div class="cdb-bracket">${CLASS_FASES_MATA_MATA.map(f => classBracketColunaHtml(f, confrontos)).join("")}</div>`;
  } catch (e) {
...
function classBracketColunaHtml(fase, confrontos) {
  ...
  return `
    <div class="cdb-bracket-coluna">
      <h3>${CLASS_NOME_FASE_MATA_MATA[fase]}</h3>
      ${confrontosFase.map(classBracketConfrontoCardHtml).join("")}
    </div>
  `;
}

function classBracketConfrontoCardHtml(confronto) {
  ...
  return `
    <div class="cdb-confronto-card ${vencedor ? 'cdb-definido' : ''}" onclick="classAbrirPrimeiroJogoBracket('${confronto.id}')">
...
async function classAbrirPrimeiroJogoBracket(confrontoId) {
