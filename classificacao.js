...
const LEGENDA_ZONAS_LIBERTADORES = `
  <div class="flex-gap" style="align-items:center;margin-bottom:10px;">
    <span style="width:10px;height:10px;border-radius:3px;background:var(--grama);display:inline-block;"></span>
    <span class="text-dim" style="font-size:12.5px;">1º e 2º — Classificados às oitavas</span>
  </div>
  <div class="flex-gap" style="align-items:center;margin-bottom:10px;">
    <span style="width:10px;height:10px;border-radius:3px;background:var(--azul);display:inline-block;"></span>
    <span class="text-dim" style="font-size:12.5px;">3º — Rebaixado para a Sul-Americana</span>
  </div>
  <div class="flex-gap" style="align-items:center;">
    <span style="width:10px;height:10px;border-radius:3px;background:var(--vermelho);display:inline-block;"></span>
    <span class="text-dim" style="font-size:12.5px;">4º — Eliminado</span>
  </div>
`;

const LEGENDA_ZONAS_SULAMERICANA = `
  <div class="flex-gap" style="align-items:center;margin-bottom:10px;">
    <span style="width:10px;height:10px;border-radius:3px;background:var(--grama);display:inline-block;"></span>
  ...
  <div class="flex-gap" style="align-items:center;">
    <span style="width:10px;height:10px;border-radius:3px;background:#2dd4bf;display:inline-block;"></span>
    <span class="text-dim" style="font-size:12.5px;">2º — Vai para o playoff (contra o 3º da Libertadores)</span>
  </div>
`;
...
async function carregarTabela() {
  ...
  if (legenda) {
    ...
    if (!ehGrupos) legenda.innerHTML = LEGENDA_ZONAS_PONTOS_CORRIDOS;
    else legenda.innerHTML = competicao?.slug === "libertadores" ? LEGENDA_ZONAS_LIBERTADORES : LEGENDA_ZONAS_SULAMERICANA;
  }
  ...
  if (ehGrupos) {
    ...
    await carregarClassificacaoPorGrupos(temporada, gruposArea, competicao?.slug);
    return;
  }
...
// ---------------------------------------------------------
async function carregarClassificacaoPorGrupos(temporada, area, slugCompeticao) {
  area.innerHTML = `<div class="empty-state"><div class="icon">⏳</div><h3>Carregando...</h3></div>`;

  const ehLibertadores = slugCompeticao === "libertadores";
  ...
  area.innerHTML = grupos.map(grupo => {
    ...
    const linhasHtml = linhas.map((l, index) => {
      // Regulamento oficial CONMEBOL:
      // - 1º: direto às oitavas (as duas competições)
      // - 2º: vai para o playoff das oitavas (as duas competições)
      // - 3º: na Libertadores desce pra Sul-Americana (ainda com chance,
      //   também via playoff); na Sul-Americana já está eliminado
      // - 4º: eliminado nas duas
      // Regulamento oficial CONMEBOL:
      // - Libertadores: 1º e 2º avançam direto às oitavas (mesma cor,
      //   não há playoff entre eles); 3º desce pra Sul-Americana (ainda
      //   com chance, via playoff); 4º eliminado.
      // - Sul-Americana: 1º avança direto às oitavas; 2º vai para o
      //   playoff (contra o 3º da Libertadores); 3º e 4º eliminados.
      let classeZona = "";
      if (ehLibertadores) {
        if (index === 0 || index === 1) classeZona = "zona-g4";
        else if (index === 2) classeZona = "zona-sul-americana";
      } else {
