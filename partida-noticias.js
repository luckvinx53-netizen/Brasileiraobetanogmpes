...
async function buscarFimDeJogoComoNoticias() {
  const temporada = typeof getTemporadaAtiva === "function" ? await getTemporadaAtiva() : null;
  if (!temporada) {
    console.warn("[fim-de-jogo] Nenhuma temporada ativa encontrada — notícias de fim de jogo não serão exibidas.");
    return [];
  }
  ...
  if (error) {
    console.error("[fim-de-jogo] Erro ao buscar jogos encerrados:", error);
    return [];
  }

  console.log(`[fim-de-jogo] Temporada ativa: ${temporada.id} — jogos encerrados encontrados: ${data?.length ?? 0}`);
  ...
  try {
    ...
    console.error("[fim-de-jogo] Erro ao converter jogo em notícia:", e);
    return [];
  }
}
