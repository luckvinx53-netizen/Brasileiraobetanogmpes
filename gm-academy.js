...
async function gmIniciar() {
  ...
  // o Supabase ainda está restaurando a sessão salva do navegador —
  // assim a pessoa não vê o botão "Entrar" piscar na tela antes de o
  // sistema perceber que ela já está logada.
  gmMostrarTela("carregando");

  // onAuthStateChange dispara assim que o supabase-js termina de
  // reidratar a sessão guardada no navegador (evento INITIAL_SESSION),
  // e também sempre que a pessoa loga/desloga depois. É a forma mais
  // confiável de saber que já dá pra confiar no estado da sessão —
  // mais confiável do que só chamar getSession() uma vez no load.
  supabaseClient.auth.onAuthStateChange((_evento, session) => {
    ...
    if (gmJaIniciou && gmTecnicoAtual?.user_id === session.user.id) return;
