...
// ---------- COBERTURA MÚLTIPLA (vários veículos cobrindo o MESMO fato) ----------
// Igual a uma busca no Google Notícias: uma transferência de peso não
// é notícia de um veículo só, é "furada"/replicada por vários ao
// mesmo tempo, cada um com sua manchete própria. Aqui decidimos
// QUANTOS veículos cobrem cada transferência confirmada (a relevância
// da notícia decide isso) e QUAIS — sempre de forma determinística
// por id da transferência, pra não mudar a cada refresh.

// Relevância = fatores objetivos do próprio negócio: perfil do
// jogador (artilheiro/joia puxam mais cobertura que reserva), se o
// valor foi divulgado e o tamanho dele. Não inventa nada que não
// esteja nos dados reais da transferência.
function tmRelevanciaTransferencia(c, jogador) {
  let pontos = 0;
  const perfil = tmPerfilJogador(jogador);
  if (perfil === "artilheiro" || perfil === "garcom") pontos += 2;
  if (perfil === "joia") pontos += 1;
  if (perfil === "veterano") pontos += 1;
  if (perfil === "reserva") pontos -= 1;

  const valor = Number(c.valor_consultado) || 0;
  if (valor >= 20000000) pontos += 3;
  else if (valor >= 8000000) pontos += 2;
  else if (valor > 0) pontos += 1;

  return pontos; // aproximadamente -1 a 6
}

// Converte a pontuação de relevância em quantidade de veículos que
// cobrem a mesma transferência: pouco relevante fica com 2 (mínimo,
// pra já ter "outras coberturas" pra mostrar), muito relevante vai
// até o total de veículos fictícios disponíveis (6).
function tmQtdCoberturas(pontosRelevancia) {
  const min = 2;
  const max = MC_VEICULOS_FALLBACK.length; // 6
  const qtd = min + Math.max(0, pontosRelevancia); // cada ponto = +1 veículo
  return Math.max(min, Math.min(max, qtd));
}

// Escolhe N veículos distintos (sem repetir) pra cobrirem a mesma
// transferência, determinístico por id — embaralha MC_VEICULOS_FALLBACK
// com um hash próprio por posição e pega os N primeiros.
function tmVeiculosParaCobertura(consultaId, qtd) {
  const candidatos = MC_VEICULOS_FALLBACK.map((nome, i) => ({
    nome,
    ordem: tmHashString(consultaId + "-cobertura-" + i + "-" + nome),
  })).sort((a, b) => a.ordem - b.ordem);

  return candidatos.slice(0, qtd).map(c => c.nome);
}

// Gera as N "coberturas" (uma por veículo) da mesma transferência
// confirmada: cada uma tem sua própria manchete (sorteada dentro do
// banco de frases do perfil, variando a semente por veículo pra não
// repetir a mesma frase) e seu próprio horário de publicação — os
// primeiros veículos "furam" primeiro, os demais replicam a notícia
// nas horas seguintes, igual cobertura de imprensa de verdade.
function tmCoberturasConfirmada(c, jogador, timeDono, timeInteressado) {
  const jogadorNome = jogador?.nome || "Jogador";
  const relevancia = tmRelevanciaTransferencia(c, jogador);
  const qtd = tmQtdCoberturas(relevancia);
  const veiculos = tmVeiculosParaCobertura(c.id, qtd);
  const publicadoEm = new Date(c.respondido_em || c.criado_em);

  return veiculos.map((nomeVeiculo, i) => {
    const seedFrase = c.id + "-cobertura-frase-" + nomeVeiculo;
    const perfil = tmPerfilJogador(jogador);
    const lista = TM_FRASES_CONFIRMADA_POR_PERFIL[perfil] || TM_FRASES_CONFIRMADA_POR_PERFIL.padrao;
    const idx = tmHashString(seedFrase) % lista.length;
    const titulo = lista[idx](jogadorNome, timeDono, timeInteressado, jogador || {});

    // Cada veículo "publica" alguns minutos/horas depois do anterior
    // (o primeiro é quem furou; os outros replicam a notícia) —
    // incremento também determinístico, entre 10min e ~5h por posição.
    const atrasoMinutos = i === 0 ? 0 : 15 * i + (tmHashString(seedFrase + "-atraso") % 90);
    const dataPublicacao = new Date(publicadoEm.getTime() + atrasoMinutos * 60000);
...
// formato de card de notícia. Quando o time interessado tem repórter
// fixo cadastrado, a matéria dele continua sendo a "principal" (como
// já era). Além dela, geramos as coberturas extras de veículos
// genéricos (tmCoberturasConfirmada) — cada uma vira o SEU PRÓPRIO
// card de notícia, com manchete e horário próprios, pra imitar uma
// busca de verdade onde o mesmo fato aparece coberto por vários sites
// diferentes (ver LEIA-ME-cobertura-multipla-noticias.md).
async function buscarConfirmadasComoNoticias() {
  ...
  data.forEach(c => {
    ...
    if (c.valor_consultado) metaPartes.push(`R$ ${Number(c.valor_consultado).toLocaleString("pt-BR")}`);
    if (c.tipo_contratacao) metaPartes.push(tipoLabel[c.tipo_contratacao] || c.tipo_contratacao);
    const resumo = metaPartes.join(" · ");

    const assinaturaPrincipal = tmAssinaturaMateria(c.id, timeInteressado);

    // Card principal — igual ao comportamento de antes, sem parâmetro
    // de veículo na URL (materia.html cai no comportamento padrão).
    resultado.push({
      origem: "confirmada",
      id: `confirmada-${c.id}`,
      consultaId: c.id,
      ...
      assinatura: assinaturaPrincipal,
      tipoContratacao: c.tipo_contratacao,
      valor: c.valor_consultado,
      grupoId: c.id, // liga esse card às outras coberturas da mesma transferência
    });

    // Coberturas extras (veículos genéricos) — não duplica o veículo
    // já usado na assinatura principal, se ela também for do tipo
    // "veiculo" (evita duas manchetes idênticas do mesmo site).
    const coberturas = tmCoberturasConfirmada(c, c.jogadores, timeDono, timeInteressado)
      .filter(cob => !(assinaturaPrincipal.tipo === "veiculo" && assinaturaPrincipal.nome === cob.veiculo));

    coberturas.forEach(cob => {
      resultado.push({
        origem: "confirmada",
        id: `confirmada-${c.id}-${cob.veiculo}`,
        consultaId: c.id,
        jogadorId: c.jogador_id,
        timeDonoId: c.time_dono_id,
        timeInteressadoId: c.time_interessado_id,
        ...
        resumo,
        tag: "Transferência confirmada",
        assinatura: { tipo: "veiculo", nome: cob.veiculo },
        tipoContratacao: c.tipo_contratacao,
        valor: c.valor_consultado,
        grupoId: c.id,
        veiculoParam: cob.veiculo, // usado pra montar o link materia.html?...&veiculo=X
      });
    });
  });
