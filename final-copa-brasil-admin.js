...
// Ao escolher o time da casa, preenche automaticamente o campo "Local"
// com o estádio cadastrado desse time (tabela times.estadio), já que a
// final é sempre mandada pelo time da casa selecionado. Só sobrescreve
// o campo se ele estiver vazio ou ainda contiver o estádio do mandante
// anterior — assim não apaga uma edição manual feita de propósito.
let fcbUltimoEstadioAutoPreenchido = "";

function fcbPreencherEstadioMandante() {
  const casaId = document.getElementById("finalTimeCasa").value;
  const campoLocal = document.getElementById("finalLocal");
  const time = fcbTimesCache.find(t => t.id === casaId);
  const estadio = time ? (time.estadio || "") : "";

  const valorAtual = campoLocal.value.trim();
  if (!valorAtual || valorAtual === fcbUltimoEstadioAutoPreenchido) {
    campoLocal.value = estadio;
    fcbUltimoEstadioAutoPreenchido = estadio;
  }
...
async function fcbCarregarFinal() {
  ...
  if (data) {
    ...
    fcbUltimoEstadioAutoPreenchido = data.local || "";
...
// botão manual "Sortear árbitros" quanto pelo sorteio automático ao
// salvar um jogo novo.
...
function fcbSortearArbitragem() {
  if (!fcbArbitrosCbfCache.length) {
    notificar("Nenhum árbitro cadastrado. Cadastre em Central Admin > Times > Árbitros CBF.", "aviso");
    return;
  }

  const { arbitro, assistente_1, assistente_2, quarto_arbitro } = fcbGerarQuartetoArbitragem();

  if (!arbitro || !assistente_1 || !assistente_2 || !quarto_arbitro) {
    notificar("Faltam árbitros cadastrados em alguma categoria (central/assistente/vídeo).", "aviso");
  }

  document.getElementById("finalArbitro").value = arbitro;
  document.getElementById("finalAssistente1").value = assistente_1;
  document.getElementById("finalAssistente2").value = assistente_2;
  document.getElementById("finalQuartoArbitro").value = quarto_arbitro;

  notificar("Árbitros sorteados! Clique em \"Salvar dados da final\" para confirmar.");
}
...
async function fcbSalvarFinal() {
  ...
  // Rede de segurança: se o campo Local ficou vazio (ex: usuário escolheu
  // o time da casa mas o onchange não rodou a tempo), preenche com o
  // estádio cadastrado do time da casa antes de salvar.
