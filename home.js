...
async function carregarHome() {
  ...
  await Promise.all([
    ...
    ehMataMata ? carregarBracketResumoHome(temporada) : carregarClassificacaoResumoHome(temporada, competicao),
  ]);
}
...
async function carregarClassificacaoResumoHome(temporada, competicao) {
  ...
  if (ehGrupos) {
    // Resumo por grupos: mostra o líder de cada grupo (1º colocado),
    // que é quem já garante vaga direta às oitavas.
    const { data: grupos, error } = await supabaseClient
      .from("grupos")
      .select("*, grupos_classificacao(*, times(*))")
      .eq("temporada_id", temporada.id)
    ...
    if (!grupos || !grupos.length) {
      tabela.innerHTML = `<tr><td colspan="8">Os grupos ainda não foram sorteados.</td></tr>`;
      return;
    }

    tabela.innerHTML = grupos.map(grupo => {
      const linhas = (grupo.grupos_classificacao || []).slice()
        .sort((a, b) => b.pontos - a.pontos || b.saldo - a.saldo || b.gols_pro - a.gols_pro);
      const lider = linhas[0];
      if (!lider) return `<tr><td colspan="8">${grupo.nome}: sem times ainda.</td></tr>`;
      return `
        <tr onclick="location.href='time?id=${lider.times.id}'" style="cursor:pointer;">
          <td>${grupo.nome}</td>
          <td>${escudoHtml(lider.times).replace('class="escudo"', 'class="escudo" style="width:22px;height:22px;"')} ${lider.times.nome}</td>
          <td>${lider.pontos}</td>
          <td>${lider.jogos}</td>
          <td>${lider.vitorias}</td>
          <td>${lider.empates}</td>
          <td>${lider.derrotas}</td>
          <td>${lider.saldo}</td>
        </tr>
      `;
    }).join("");
