...
async function carregarMateriaFimDeJogo(area, id) {
    ...
    ? `
      ...
      <ul class="materia-lista-gols">
        ${eventos.map(e => {
          ...
          const nomeAutor = e.jogador_nome || mapaJogadores[e.jogador_id]?.nome || "Autor não identificado";
          const link = e.jogador_id
            ? `<a href="jogador?id=${e.jogador_id}">${nomeAutor}</a>`
            : nomeAutor;
  ...
  // Card "Sobre o jogador" (foto/posição/estatísticas), um por autor de
  // gol distinto — mesmo componente usado nas matérias de rumor/
  // transferência (mtJogadorCardHtml). Inclui também quem fez gol
  // contra: apesar de não contar como artilheiro, foi ele quem
  // participou do lance, então também ganha o card.
  const artilheirosCards = [...new Set(
    (eventos || [])
      .filter(e => e.jogador_id)
      .map(e => e.jogador_id)
  )]
    .map(idJogador => mapaJogadores[idJogador])
    .filter(Boolean);

  const artilheirosHtml = artilheirosCards.length
    ? `
      <p class="materia-secao-titulo">${artilheirosCards.length > 1 ? "Quem balançou as redes" : "Sobre o autor do gol"}</p>
      ${artilheirosCards.map(jg => mtJogadorCardHtml(jg, jg.time)).join("")}
    `
    : "";
