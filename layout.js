...
// Mostra o item "Meu Time" na bottom-nav e troca o item "GM Academy"
// do menu "☰" por "Meu Time" quando o visitante logado for um técnico
// contratado (com time_id preenchido). Roda depois do menu já estar
...
async function adicionarLinkMeuTimeSeTecnico() {
  ...
  try {
    ...
    // o que ver em meu-time.html — a área dele fica no GM Academy.
    // Usamos só time_id (não o campo status) porque é a fonte de
    // verdade mais confiável.
    if (!vinculo || !vinculo.time_id) return;

    // Bottom-nav: adiciona o item "Meu Time" (se ainda não estiver lá)
    const nav = document.querySelector(".bottom-nav");
    if (nav && !nav.querySelector('a[href="meu-time.html"]')) {
    ...
    const itemGmAcademy = document.querySelector('#menuExtraPainel a[href="gm-academy.html"]');
    if (itemGmAcademy) {
      itemGmAcademy.href = "meu-time.html";
