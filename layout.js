...
async function adicionarLinkMeuTimeSeTecnico() {
  ...
  try {
    ...
    const { data: vinculo } = await supabaseClient
      .from("tecnicos")
      .select("user_id, time_id, status")
      .eq("user_id", session.user.id)
      .maybeSingle();

    // Só mostra "Meu Time" para quem está de fato contratado (com
    // time_id). Um técnico licenciado (sem clube) não tem o que ver
    // em meu-time.html — a área dele fica no GM Academy.
    if (!vinculo || !vinculo.time_id || vinculo.status === "licenciado") return;
