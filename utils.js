...
async function checarEncerramentoAutomatico(jogo) {
  ...
  // Jogo já estava Encerrado ANTES da rede social existir: nunca vai
  // passar de novo pelo bloco de encerramento abaixo (é executado só
  // uma vez, no momento em que o jogo termina). Para não deixar esses
  // jogos antigos sem post pra sempre, tenta gerar o post de fim de
  // jogo aqui mesmo — rsGerarPostFimDeJogoSeNecessario() é idempotente
  // (índice único no banco), então rodar isso toda vez que a página
  // carrega não duplica nada; só cria o post na primeira vez que
  // alguém visita a página depois de termos essa funcionalidade.
